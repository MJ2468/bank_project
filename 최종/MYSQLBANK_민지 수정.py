...
def search_account(account): #12345
    search = "SELECT 계좌번호 FROM 고객정보 WHERE 계좌번호=%s"
    list1 = [] # 계좌번호
    conn = getCon()
    with conn:
        with conn.cursor() as cur :
            ...
            result = cur.fetchall() 
            for data in result:
                list1 += list(data)  # list1 = [계좌번호, 예금주, 생년월일, 비밀번호, 잔액]
            
        while account == list1[0]: # true
            print('존재하는 계좌번호입니다. 다른 번호를 입력하세요') # 계속 시행
            break
...
print(type(search_account('555-555')))
